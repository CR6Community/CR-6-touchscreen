# Release#: 61F_220622_v1

## NOTE: If you do not see this release number on the Setup->Info screen of your system, you have not flashed this version of the touchscreen firmware. 

# Introduction

This refactored firmware operates correctly on all versions of DGUS2 OS.
It is no longer necessary to install kernel upgrade files to reprogram the display to use DGUS2 v3.5.  [BUT: IF your display currently runs a version of DGUS2 less than v3.5, you may observed font kerning issues (e.g. the bed mesh values will not fit inside the boxes)  Flashing the DGUS2 v3.5 kernel upgrade file will correct that problem.]

This formal release is derived from 61F_110622_v1.

NEW AT THIS RELEASE:

- The colour scheme has been revised, to a Cyanotype theme.  
	- This was done partly to refresh the "look" and partly to help our Community memebers to easily distinguish which version of the firmware is pictured in various "calls for help" on the Discord and Facebook channels.
	- Knowing that aesthetics can be very individual, I apologize to anyone who finds this scheme "ugly".  I experimented with a lot of other schemes before settling onto this one. Making these conversions takes me days at a time, and frankly I ran out of steam when this one looked "crisp and elegant enough..."

- Found and eliminated a small number of errors with units and data field alignment inconsistencies found through testing the RC versions.
- Found and fixed a couple of bugs that snuck back into the firmware, possibly through a loss of change control with GitHub 

- Added some more sample screen images, to show the new Thermal Warning screens.

DEPRECATED AT THIS RELEASE:
Just to repeat for CR-MAX users:
...
Following the CF6.1 installation instructions to flash v3.5 with CF6.1 to these newer printers therefore causes the v4.5 screen calibration to be invalid. 
- That results in the touchscreen becoming non-responsive to CF6.1. 
- Reverting to Creality stock also then fails, until the DGUS2 v4.5 kernel is restored to the display.
...
2. The display hardware has already been calibrated to operate with the version of DGUS2 (2.0 or higher) that is installed on the display.
    (NOTE: this firmware also runs on DGUS2v1.4, but with font kerning issues)
...
### If you already flashed the CF6.1-Final touchscreen firmware to your display hardware, your display is now running DGUS2 v3.5, and your display is non-responsive (meaning that tapping on the screen may make a beeping sound but none of the buttons work)
